...
int receive_and_send(int connfd1, int connfd2)
{
    ...
    while (bytes_remaining > 0)
    {
        ...
        DIE(bytes_send < 0, "Bytes unsuccessfully sent to client 2");
...
void run_echo_server(int listenfd)
{
    ...
    int result = listen(listenfd, 1);
    DIE(result < 0, "Failure in listening connections");

    // TODO 5: Acceptati o conexiune
    connfd = accept(listenfd, (struct sockaddr *)&client_addr, &socket_len);
    DIE(connfd < 0, "Failure in accepting connection");
...
int main(int argc, char *argv[])
{
    ...
    DIE(listenfd < 0, "Failure in creation of socket");
    ...
    int result = inet_aton(argv[1], &serv_addr.sin_addr);
    DIE(result == 0, "Address is not valid");
    ...
    DIE(b == -1, "Error binding name to socket");
