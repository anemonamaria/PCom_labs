...
void ping(int sockfd, struct in_addr ip_addr, int count)
{
	...
	for(i = 0; i < count; i++) {
		...
		printf("Message from: %s, sequence: %hu, TTL: %u, time: %1.f ms\n",
...
void traceroute(int sockfd, struct in_addr ip_addr)
{
	...
	while(destination == 0) {
		...
		icmp_hdr->type = 8;
		icmp_hdr->code = 0;
		icmp_hdr->un.echo.sequence = htons(hop_router_idx);
		icmp_hdr->un.echo.id = htons(getpid());
		icmp_hdr->checksum = 0;
		icmp_hdr->checksum = checksum(icmp_hdr, sizeof(struct icmphdr));
		...
		printf("%2d %s %.03f ms\n", hop_router_idx, inet_ntoa(addr), time_elapsed);

		destination = (ricmp_hdr->type != ICMP_TIMXCEED) ? 1 : 0;
		hop_router_idx++;
	}
}
